...
export class DatingSimulator {
  ...
  chooseChatOption(optionIndex) {
    ...
    if (roll < option.successProb) {
      ...
      // Sugar mommy rewards or dating upgrades
      if (this.currentProfile.name === 'Gertrude') {
        this.player.confidence = Math.max(0, this.player.confidence - 45); // lost pride
        this.logCallback("You traded your youthful essence for Gertrude's fortune (-45% Confidence, +$5,000 cash).", "success");
      } else {
