...
export const ACHIEVEMENTS = [
  { id: 'first_roll', name: 'Born', desc: 'Roll your genetics for the first time.', icon: '👶', check: (p) => true, progress: () => 100 },
  { id: 'millionaire', name: 'Cash King', desc: 'Accumulate $10,000 cash.', icon: '💰', check: (p) => p.cash >= 10000, progress: (p) => Math.min(100, Math.round(p.cash / 10000 * 100)) },
  { id: 'gigachad', name: 'GigaChad/GigaStacy', desc: 'Reach SMV 7.5+', icon: '👑', check: (p) => p.smv >= 7.5, progress: (p) => Math.min(100, Math.round(p.smv / 7.5 * 100)) },
  { id: 'truecel', name: 'Truecel', desc: 'Reach rock bottom.', icon: '😭', hidden: 'The threshold is lower than you think.', check: (p) => p.smv <= 2.5, progress: (p) => p.smv <= 2.5 ? 100 : 0 },
  { id: 'influencer', name: 'TikTok Famous', desc: 'Reach 10,000 followers.', icon: '📱', check: (p) => p.followers >= 10000, progress: (p) => Math.min(100, Math.round(p.followers / 10000 * 100)) },
  { id: 'surgery_survivor', name: 'Knife Magnet', desc: 'Survive 3+ botched surgeries.', icon: '💉', check: (p) => p.surgeryBotchedCount >= 3, progress: (p) => Math.min(100, Math.round(p.surgeryBotchedCount / 3 * 100)) },
  { id: 'heartbreaker', name: 'Heartbreaker', desc: 'Someone actually wants you.', icon: '💔', hidden: 'A romantic milestone.', check: (p) => p.hasDatingPartner, progress: (p) => p.hasDatingPartner ? 100 : 0 },
  { id: 'mogger', name: 'Ultimate Mogger', desc: 'Defeat all opponents in battle.', icon: '🏆', check: (p) => p.opponentsDefeated.length >= 12, progress: (p) => Math.min(100, Math.round(p.opponentsDefeated.length / 12 * 100)) },
  { id: 'ceo_grind', name: 'CEO Grindset', desc: 'Reach CEO career tier.', icon: '💼', check: (p) => p.careerTier === 'ceo', progress: (p) => { const idx = ['unemployed','entry','junior','mid','senior','manager','director','executive','ceo'].indexOf(p.careerTier); return Math.min(100, Math.round(idx / 8 * 100)); } },
  { id: 'surgeon_savvy_ach', name: 'Natural Beauty', desc: 'Keep your original parts.', icon: '🌿', hidden: 'A full run without enhancements.', check: (p) => p.surgeryBotchedCount === 0, progress: (p) => p.surgeryBotchedCount === 0 ? 100 : 0 },
  { id: 'addict', name: 'Substance Abuser', desc: 'Take 5+ substances in one run.', icon: '💊', check: (p) => p.substancesUsed >= 5, progress: (p) => Math.min(100, Math.round(p.substancesUsed / 5 * 100)) },
  { id: 'centenarian', name: 'Elder', desc: 'Reach age 50.', icon: '👴', check: (p) => p.age >= 50, progress: (p) => Math.min(100, Math.round(p.age / 50 * 100)) },
  { id: 'rizzler', name: 'Rizz God', desc: 'Reach 100 Rizz.', icon: '🔥', check: (p) => p.rizz >= 100, progress: (p) => Math.min(100, p.rizz) },
  { id: 'rival_slayer', name: 'Rival Slayer', desc: 'Dominate your rival at every meeting.', icon: '🥊', hidden: 'There are several rival encounters throughout life.', check: (p) => p.rivalDefeatedCount >= 4, progress: (p) => Math.min(100, Math.round(p.rivalDefeatedCount / 4 * 100)) },
  { id: 'challenge_complete', name: 'Challenge Mode', desc: 'Complete any challenge run.', icon: '🏅', hidden: 'Select a challenge at birth and finish the run.', check: (p) => p.completedChallenge, progress: (p) => p.completedChallenge ? 100 : 0 },
  { id: 'style_max', name: 'Style Icon', desc: 'Reach 100 Style.', icon: '👔', check: (p) => p.style >= 100, progress: (p) => Math.min(100, p.style) },
  { id: 'frame_max', name: 'Giga Frame', desc: 'Reach 100 Frame.', icon: '💪', check: (p) => p.frame >= 100, progress: (p) => Math.min(100, p.frame) },
  { id: 'skin_max', name: 'Porcelain Skin', desc: 'Reach 100 Skin.', icon: '✨', check: (p) => p.skin >= 100, progress: (p) => Math.min(100, p.skin) },
  { id: 'max_confidence', name: 'Unshakeable', desc: 'Reach 100 Confidence.', icon: '🧠', check: (p) => p.confidence >= 100, progress: (p) => Math.min(100, p.confidence) },
  { id: 'high_roller', name: 'High Roller', desc: 'Accumulate $20,000 cash.', icon: '💎', check: (p) => p.cash >= 20000, progress: (p) => Math.min(100, Math.round(p.cash / 20000 * 100)) },
  { id: 'death_surgery', name: 'Table Fare', desc: 'A fatal mistake on the table.', icon: '🩸', hidden: 'Some surgeons are better than others.', check: (p) => p.isDead && p.surgeryBotchedCount > 0, progress: (p) => (p.isDead && p.surgeryBotchedCount > 0) ? 100 : 0 },
  { id: 'death_overdose', name: 'Overdose', desc: 'Push substances too far.', icon: '☠️', hidden: 'Everything in moderation... or not.', check: (p) => p.isDead && p.log.some(l => l.includes('FATAL')), progress: (p) => 0 },
  { id: 'substance_max', name: 'Poly-Drug User', desc: 'Take 15+ substances in one run.', icon: '💊', check: (p) => p.substancesUsed >= 15, progress: (p) => Math.min(100, Math.round(p.substancesUsed / 15 * 100)) },
  { id: 'baby_maker', name: 'Legacy', desc: 'Have a child.', icon: '👶', check: (p) => p.hasProcreated, progress: (p) => p.hasProcreated ? 100 : 0 }
];
